...
int SlottedPage::GetTupleOffsetForDemo(int slot_id) const {
    if (slot_id < 0 || slot_id >= GetSlotCount()) {
        return -1;
    }

    return GetSlotOffset(slot_id);
}

int SlottedPage::GetTupleSizeForDemo(int slot_id) const {
    if (slot_id < 0 || slot_id >= GetSlotCount()) {
        return -1;
    }

    int size = GetSlotSize(slot_id);
