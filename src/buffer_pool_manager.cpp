...
Page* BufferPoolManager::FetchPage(page_id_t page_id) {
    ...
    std::cout << "[DISK READ] Loaded page_id=" << page_id
              << " into frame=" << frame_id
              << std::endl;
...
Page* BufferPoolManager::NewPage(page_id_t* page_id) {
    ...
    if (frame_id == -1) {
        std::cout << "[NEW PAGE FAILED] No available frame. All pages are pinned."
                  << std::endl;
        return nullptr;
    }
    ...
    // DiskManager::AllocatePage writes an empty page to disk.
...
bool BufferPoolManager::FlushFrame(int frame_id) {
    ...
    std::cout << "[DISK WRITE] Flushed page_id=" << frame.page_id
              << " from frame=" << frame_id
              << " to disk"
              << std::endl;
